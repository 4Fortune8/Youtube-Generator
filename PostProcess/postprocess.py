...
def postprocess(reveservideopath,forwardvideopath,finalpath):
    ...
    for image_file in image_files:
        command = f'.\\realcugan-ncnn-vulkan-20220728-windows\\realcugan-ncnn-vulkan.exe -i {os.path.join(input_dir, image_file)} -o {os.path.join(output_dir, image_file)} -g 0  -j 4:4:4'
        subprocess.run(command, shell=True)
    ...
    for pos,image_file in enumerate(image_files[:-1]):
        command = f'.\\rife-ncnn-vulkan-20221029-windows\\rife-ncnn-vulkan.exe -m rife-v4.6 -0 {os.path.join(input_dir, image_files[pos])} -1 {os.path.join(input_dir, image_files[pos+1])} -o {os.path.join(input_dir, f"{pos+1:03}_newframe.png" )} -g 0 -j 4:4:4 -n {startframes*2} '
        subprocess.run(command, shell=True)
...
postprocess("video1.mp4","video2.mp4","outputvideo.mp4")