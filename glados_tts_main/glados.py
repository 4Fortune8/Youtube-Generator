...
import torch
import numpy as np
from glados_tts_main.utils.tools import prepare_text
...
class tts_runner:
    def __init__(self, use_p1: bool=False, log: bool=False):
        self.log = log
        if use_p1:
            self.emb = torch.load('glados_tts_main/models/emb/glados_p1.pt')
        else:
            self.emb = torch.load('glados_tts_main/models/emb/glados_p2.pt')
        # Select the device
        ...
        # Load models
        self.glados = torch.jit.load('glados_tts_main/models/glados-new.pt')
        self.vocoder = torch.jit.load('glados_tts_main/models/vocoder-gpu.pt', map_location=self.device)
        for i in range(2):
    ...
    def speak(self, text,newpath, alpha: float=1.0, save: bool=False, delay: float=0.1):
...
def makeGladosAudio(folderName, text):
    ...
    try:
        ...
        pass
    glados.speak(text,newpath, True)
